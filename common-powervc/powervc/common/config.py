...
# without conflicting with other options parsing as before, particularly those
# OpenStack AMQP configurations in [DEFAULT] section of nova.conf, cinder.conf.
argv = sys.argv[1:]
argv1 = None
argv2 = None
argv1_index = -1
...
for i in range(len(argv)):
    if argv[i] == '--config-file':
        if os.path.basename(argv[i+1]) == 'amqp-openstack.conf':
            argv1 = argv[i:i+2]
            argv1_index = i+1
