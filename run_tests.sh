...
function usage {
  echo "Usage: $0 [OPTION]..."
  echo "Run PowerVC Driver test suite(s)"
  echo ""
...
function run_tests {
  ...
  COMPONENTS=(`ls | grep powervc`)
  NOSETESTS_TARGETEDDIR=""
  for COMPONENT in ${COMPONENTS[@]} ; do
    NOSETESTS_TARGETEDDIR=${NOSETESTS_TARGETEDDIR}" $COMPONENT/"    
  done
  ${wrapper} $NOSETESTS $NOSETESTS_TARGETEDDIR
  # If we get some short import error right away, print the error log directly
  
  #ARGETED_COMPONENTS=(`ls | grep powervc`)
  #or TARGETED_COMPONENT in ${TARGETED_COMPONENTS[@]} ; do
  # ${wrapper} $NOSETESTS $TARGETED_COMPONENT/
  #one
...
function run_flake8 {
  ...
  #echo ${wrapper} flake8 $FLAGS $commonent/powervc | tee pep8.txt
  TARGETED_COMPONENTS=(`ls | grep powervc`)
  for TARGETED_COMPONENT in ${TARGETED_COMPONENTS[@]} ; do
    ${wrapper} flake8 $FLAGS $TARGETED_COMPONENT/powervc | tee pep8.txt
  done
  RESULT=${PIPESTATUS[0]}
  return $RESULT
}
